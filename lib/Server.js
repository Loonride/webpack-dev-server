...
class Server {
  ...
  constructor(compiler, options = {}, _log) {
    ...
    if (options.inline !== false) {
      // const hasHMRPlugin = (config) => {
      //   if (!config.plugins) {
      //     return false;
      //   }

      //   if (!config.plugins.find(
      //       (plugin) =>
      ...
      // if ([].concat(webpackConfig).find(
      //     (config) => !hasHMRPlugin(config)
      //   )
      // ) {
      ...
      // a quick, dirty way to check if this webpack config passed
      // through addEntries
      const passedThroughEntries = (config) => {
        if (!config.entry || !config.plugins) {
          return false;
        }
        const sockPathTest = new RegExp(/webpack-dev-server/);
        if (
          ![].concat(config.entry).find((entry) => sockPathTest.test(entry))
        ) {
          return false;
        }

        if (
          (options.hot || options.hotOnly) &&
          !config.plugins.find(
            (plugin) =>
              plugin.constructor === webpack.HotModuleReplacementPlugin
          )
      ...
      let configsPassed = true;
      let webpackConfig;
      if (compiler.compilers) {
        webpackConfig = [];
        compiler.compilers.forEach((compiler) => {
          const clonedOptions = clone(compiler.options);
          webpackConfig.push(clonedOptions);
          if (configsPassed && !passedThroughEntries(clonedOptions)) {
            configsPassed = false;
          }
        });
      } else {
        webpackConfig = clone(compiler.options);
        if (!passedThroughEntries(webpackConfig)) {
          configsPassed = false;
        }
      }

      if (!configsPassed) {
        // force the webpack config to pass through addEntries
        // and create a new compiler
